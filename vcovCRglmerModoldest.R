...
#      FF = solve(diag(ng) - t(H))
      WB_A = diag(ng)
      WB_U = -t(Vinv)%*%X[grp,]
      WB_V = t(X[grp,])
#      W = WoodburyMatrix(A = WB_A, B = WB_C1, U = WB_U, V = WB_V)
#      FF = solve(W)
...
#
#    sum = sum + AA%*%t(X[grp,])%*%Vinv%*%t(FF)%*%e[grp,,drop=FALSE]%*%t(e[grp,,drop=FALSE])%*%FF%*%Vinv%*%X[grp,]%*%AA
  }
  c = 1
  if (type1=="DF") {
